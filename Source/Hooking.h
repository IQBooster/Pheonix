...
typedef bool(__cdecl* PickupProtSig)(void* classidonthave, void* unk, CPed* ped);
typedef void(__cdecl* SetWarningMessage2)(char* entryHeader, char* entryLine1, int instructionalKey, char* entryLine2, BOOL p4, Any p5, Any* p6, Any* p7, BOOL background);
typedef void(__cdecl* SetSessionWeather)(int eventGroup, int weatherID, int a3, uint64_t alwaysZero);
typedef void(__cdecl* SetSessionTime)(int, int);
typedef void(__cdecl* TriggerScriptEvent)(int eventGroup, uint64_t* args, int argCount, int bit);
typedef BOOL(__cdecl* GetEventData)(std::int32_t eventGroup, std::int32_t eventIndex, std::int64_t* args, std::uint32_t argCount);
typedef uint32_t*(__cdecl* FileRegister) (int* textureID, const char* texturePath, bool unk3, const char* texture, bool unk5);
typedef const char* (__cdecl* GetLabelText)(void* unk, const char* label);
typedef int64_t(__cdecl* GetPlayerAddress)(Player player);
...
public:
	...
	static PickupProtSig			pickup_prot_sig;
	static SetWarningMessage2		set_warning_message_2;
	static SetSessionTime			set_session_time;
	...
	//static void NativePatch(bool toggle);
	static void ownedExplosionBypass(bool toggle);
	static void modelSpawnBypass(bool toggle);
	static void modelCheckBypass(bool toggle);
